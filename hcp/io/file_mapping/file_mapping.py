...
preprocessed = {
    "meg": {
        ...
        "patterns": {
            ("epochs", "rmeg"): ["{subject}_MEG_{run}-{kind}_{context}preproc.mat"],
            ("epochs", "tmeg"): [
            ...
            "trial_info": ["{subject}_MEG_{run}-{kind}_tmegpreproc_trialinfo.mat"],
        },
    },
    "meg_anatomy": {
        ...
        "patterns": {
            ...
            "head_model": ["{subject}_MEG_anatomy_headmodel.mat"],
            "source_model": [
    ...
    "freesurfer": {
        ...
        "patterns": {"label": [], "surf": [], "mri": [], "stats": [], "touch": []},
    },
}
...
def _map_onset(onset, data_type, output):
    ...
    if data_type == "task_working_memory":
        ...
        out = {"stim": "TEV", "resp": "TRESP", "sentence": "BSENT", "block": "BUN"}[
            onset
        ]
    return out
...
def get_file_paths(
    ...
    if data_type not in kind_map:
        raise ValueError(
            ...
            f"I currently support:\n{r', '.join([k for k in kind_map if '_' in k])}"
        )
    ...
    if data_type not in ("meg_anatomy", "freesurfer"):
        ...
        if run_index >= len(my_runs):
            raise ValueError(
                "For `data_type=%s` we have %d runs. "
                "You asked for run index %d." % (data_type, len(my_runs), run_index)
            )
        run_label = my_runs[run_index]
    else:
        run_label = None
    if data_type in ("noise_subject", "noise_empty_room") and output in (
        "epochs",
        "evoked",
        "ica",
        "annot",
    ...
    if processing == "preprocessed":
        ...
        if my_pattern is None:
            raise ValueError('What is output "%s"? I don\'t know about this.' % output)
