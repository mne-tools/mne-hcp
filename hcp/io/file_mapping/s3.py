...
def get_s3_keys_meg(
    ...
    if not all(isinstance(rr, int) for rr in run_inds):
        raise ValueError(
            "Rund indices must be integers. I found: "
            + ", ".join(["%s" % type(rr) for rr in run_inds if not isinstance(rr, int)])
        )
    elif max(run_inds) > 2:
        raise ValueError(
            "For HCP MEG data there is no task with more"
