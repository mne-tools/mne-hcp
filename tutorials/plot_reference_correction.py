...
def plot_psd(X, label, Fs, NFFT, color=None):
    freqs, psd = welch(
        X, fs=Fs, window="hanning", nperseg=NFFT, noverlap=int(NFFT * 0.8)
    )
    freqs = freqs[freqs > 0]
    psd = psd[freqs > 0]
...
raw = hcp.read_raw(
    subject=subject, hcp_path=hcp_path, run_index=run_index, data_type=data_type
)
raw.load_data()
...
plot_psd(x_meg_ref, Fs=raw.info["sfreq"], NFFT=n_fft, label="MEG-REF", color="red")
plot_psd(
    raw[meg_picks][0][:, ::decim_show].mean(0),
    Fs=raw.info["sfreq"],
    NFFT=n_fft,
    label="MEG-corrected",
    color="orange",
...
plt.figure()
plt.plot(raw.times, chan1.ravel() * 1e15, label="%s before" % ch_name, color="black")
plt.plot(raw.times, chan1c.ravel() * 1e15, label="%s after" % ch_name, color="orange")
