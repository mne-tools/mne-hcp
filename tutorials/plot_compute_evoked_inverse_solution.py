...
hcp_evokeds = hcp.read_evokeds(
    onset="stim", subject=subject, data_type=data_type, hcp_path=hcp_path
)
...
raw_noise = hcp.read_raw(
    subject=subject, hcp_path=hcp_path, data_type="noise_empty_room"
)
raw_noise.load_data()
...
stc = mne.minimum_norm.apply_inverse(  # these data have a pretty high SNR and
    evoked, inv_op, method="MNE", lambda2=1.0 / 9.0**2
)  # 9 is a lovely number.

stc = stc.to_original_src(src_outputs["src_fsaverage"], subjects_dir=subjects_dir)
